...
def test_bar_chart_schema_key_fields():
    ...
    for field_name in expected_fields:
        assert field_name in properties, f"Expected field '{field_name}' not found in schema"
...
def test_bar_chart_schema_field_types():
    ...
    if "type" in title_field:
        assert title_field["type"] in ["string", "null"] or \
               isinstance(title_field["type"], list)
...
def test_bar_chart_schema_no_required_fields():
    ...
    assert len(required) == 0, \
        f"Expected no required fields, but found: {required}"
